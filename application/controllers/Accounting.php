...
class Accounting extends CI_Controller
{
    ...
    public function set_invoice(){
        ...
        $sql = "select A.*, B.sch_id, B.event_type, B.event_name, B.event_date, B.basic_cost, B.net_amount, B.other_issuer from 
                (select * from rent_txn where txn_status = 'Approved') A 
                left join 
                (select A.*, B.invoice_issuer as other_issuer 
                from rent_schedule A left join rent_other_amt_details B on (A.rent_id=B.rent_id and A.event_name=B.event_name) 
                where status = '1' and (invoice_no is null or invoice_no='') and 
        ...
        if(count($result)>0){
            for($i=0; $i<count($result); $i++){
                ...
                if(strtoupper(trim($result[$i]->event_type))=="OTHER"){
                    if(isset($result[$i]->other_issuer)){
                        if($result[$i]->other_issuer!=""){
                            $invoice_issuer = $result[$i]->other_issuer;
                        }
                    }
                }
        ...
        $sql = "select * from actual_other_schedule where txn_status = 'Approved' and type='receipt' and 
    ...
    public function get_invoice($type, $id){
        if($type=='Rent'){
            ...
            if(strtoupper(trim($event_type))=="OTHER"){
                $sql = "select * from rent_other_amt_details where rent_id = '$rent_id' and 
                        event_name = '$event_name'";
                $query = $this->db->query($sql);
                $result = $query->result();
                if(count($result)>0){
                    if(isset($result[0]->invoice_issuer)){
                        if($result[0]->invoice_issuer!=''){
                            $issuer_id = $result[0]->invoice_issuer;
                        }
                    }
                }
        ...
        if(count($invoice)>0){
            ...
            if(count($result)>0){
                if($result[0]->c_name!=''){
                    ...
                    $data['issuer_name'] = $result[0]->c_company_name;
                }
            ...
            $result = $this->db->query($sql)->result();
            if(count($result)>0){
                $data['tenant_name'] = $result[0]->c_name . ' ' . $result[0]->c_last_name. ' ' . $result[0]->c_company_name;
