<table id="contacts" class="view_table group1 addschedule">
	<thead>
		<tr>
		...
		<?php if(isset($property_details['schedule_tax_detail'])){
			...
			foreach ($property_details['schedule_tax_detail'] as $row) {
				if($property_details['schedule_tax_detail'][$i]['total_amount_paid'] == $property_details['schedule_tax_detail'][$i]['tax_amount'] ){
					if(isset($property_details['schedule_tax_detail'][$i]['amount_paid']) && $property_details['schedule_tax_detail'][$i]['amount_paid']!='') {
						$disabled='';
					} else {
						$disabled='disabled';
					}
				...
				echo '<tr class="tax_num">
						...
						<td >
							<input type="text" id="tax_actual_amount_'.($i+1).'" name="tax_actual_amount[]" class="form-control format_number" style="border: none;  " onchange="getDifferneceTaxAmt();" value="'.$property_details['schedule_tax_detail'][$i]['amount_paid_pending'].'" '.$disabled.'/>
						</td>
						<td >
