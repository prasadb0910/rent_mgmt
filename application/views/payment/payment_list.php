              			...
              			<div class="main-container">
              				<div class="col-md-12" style="padding:0;">
              					<div class="panel panel-default inside-width" style="border:none;box-shadow:none;">
              							...
              							<div class="table-responsive">
              								<table id="customers2" class="table datatable table-bordered">
              									<thead>
              										<tr>
                                    <th>Sr. No</th>
                                    <?php if(strtoupper(trim($payment_status))=='PAYMENTS_PENDING') { ?>
                                    <th>Invoice Date</th>
                                    <?php } else { ?>
                                    <th>Payment Date</th>
                                    <?php } ?>
                                    ...
                                    <?php if(strtoupper(trim($payment_status))=='PAYMENTS_PENDING') { ?>
                                      <td style="padding:5px;"><?php echo ($data[$i]->invoice_date!=null && $data[$i]->invoice_date!='')?date('d/m/Y',strtotime($data[$i]->invoice_date)):''; ?></td>
                                    <?php } else { ?>
                                    ...
                                    <?php if(strtoupper(trim($payment_status))=='PAYMENTS_PENDING') { ?>
                                    <td>
                                        <?php if($data[$i]->payment_status!='paid') { ?>
                                        <a href="<?php echo 'https://www.pecanreams.com/dataFrom.htm?user_id='.$data[$i]->user_id.'&sub_id=0&trans_id='.$data[$i]->id; ?>" class="label label-success" target="_blank">Pay Now</a>
                                        <?php } ?>
                                    </td>
                                    <?php } ?>
              										</tr>
