...
public function Income(){
    ...
    if($contact_id!="")
    {
       $where_condition .= " And E.pr_client_id='$contact_id' ";
    }
    ...
    /*$query = $this->db->query("Select p.p_property_name,p.txn_id ,a.*,sp_name,
        ...
        // where C.paid_amount>0) D where D.paid_amount>0
    $query = "Select E.* ,b_name ,em.expense_category,
            ...
            from 
            (select E.*,(Select pr_client_id from contact_master cm join purchase_ownership_details p 
            where p.purchase_id=E.property_id and cm.c_id=p.pr_client_id Limit 0,1) as pr_client_id,E.payer_id as contact_id, F.c_full_name, F.owner_name from 
            ...
            left join bank_master b on b.b_id=E.account_number
                    left join expense_category_master em on em.id = E.event_name where type='receipt'".$where_condition.' Order By event_date ASC';
    $queryresult = $this->db->query($query);  
    $resultother = $queryresult->result_array();
    $this->db->last_query();
    $result = $resultother;
    if(count($result)>0){
        for($i=0;$i<count($result);$i++){
            ...
            $category = $result[$i]['expense_category'];
            //$event_name = $result[$i]['expense_category'];
            $p_property_name = $result[$i]['p_property_name'];
            $owner_name = $result[$i]['payername'];
            $payer_name =  $result[$i]['owner_name'];
            ...
            $bal_amount = $result[$i]['bal_amount'];
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $basic_cost);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $bal_amount);*/
    ...
    $sqlsale = "Select * from (select E.*,'sales' as table_type,'' as expense_category,b_name ,
        (Select case when c_owner_type='individual' 
        then concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) 
        else concat(ifnull(c_company_name,'')) end as owner_name 
        ...
        (select A.*,(Select pr_client_id from contact_master cm join purchase_ownership_details p 
        where p.purchase_id=A.property_id and cm.c_id=p.pr_client_id Limit 0,1) as pr_client_id,B.p_property_name, B.p_display_name, B.p_type, B.p_status from 
        (select * from 
        ...
        B.id as accounting_id, B.txn_status, case when B.paid_amount is null then 0 else B.paid_amount end as paid_amount, A.entry_type ,B.payment_mode,B.cheque_no,B.account_number,B.tds_amount from 
        ...
        left join 
        (select id, fk_txn_id, event_type, event_name, event_date, (paid_amount+tds_amount) as paid_amount, txn_status ,payment_mode,cheque_no,account_number ,tds_amount
        ...
        amount_paid as paid_amount, txn_status ,payment_mode,cheque_no,account_number ,'' as tds_amount  from actual_schedule_taxes where table_type = 'sales' ) B 
        ...
        left join bank_master b on b.b_id=E.account_number) E where table_type = 'sales'  ".$where_condition.' Order By event_date ASC';
    $result = [];
    ...
    $sqlrent="Select * from (select E.*,'rent' as table_type from 
        ...
        case when D.net_amount is null then 0 else D.net_amount end as net_amount, 
        case when D.tax_amount is null then 0 else D.tax_amount end as tax_amount, 
        case when D.paid_amount is null then 0 else D.paid_amount end as paid_amount ,D.tds_amount from 
        (select A.*, B.sp_name from 
        ...
        case when B.paid_amount is null then 0 else B.paid_amount end as paid_amount,B.tds_amount from 
        ...
        left join 
        (select fk_txn_id, event_type, event_name, event_date, sum(paid_amount)+sum(tds_amount) as paid_amount, tds_amount
        from actual_schedule where table_type = 'rent' group by fk_txn_id, event_type, event_name, event_date, tds_amount) B 
        ...
        where E.owner_name is not null and E.owner_name<>'') E Where  E.table_type = 'rent' ".$where_condition.' Order By event_date ASC';
    $result = [];
    $result2=$this->db->query($sqlrent)->result_array();
    $result = $result2;
    if(count($result)>0){
        for($i=0;$i<count($result);$i++){
            /*
            ...
            //$event_name = $result[$i]['expense_category'];
            $p_property_name = $result[$i]['p_property_name'];
            $owner_name = $result[$i]['owners_name'];
            $payer_name =  $result[$i]['payer_name'];
            // $category = 'Rent';
            $category = $result[$i]['event_name'];
            $basic_cost = $result[$i]['basic_cost'];
            $tax_amount = $result[$i]['tax_amount'];
            $tds_amount = $result[$i]['tds_amount'];
            $tds_amount = $result[$i]['tds_amount'];
    ...
    if(count($resultother)>0 || count($result1)>0 || count($result2)>0){
        ...
        /* $objPHPExcel->getActiveSheet()->setCellValue('L'.$row, '=SUM(L'.$start.':'."L".($row-1).')');
        $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, '=SUM(M'.$start.':'."M".($row-1).')');
...
public function Expense(){
    ...
    $query = "Select E.* ,b_name ,em.expense_category,
            ...
            (select  D.fk_txn_id, D.type, D.event_type, D.event_name, D.event_date, D.payer_id, D.property_id, D.sub_property_id, D.txn_status, 
                ...
                case when D.net_amount is null then 0 else D.net_amount end as net_amount, 
                case when D.tax_amount is null then 0 else D.tax_amount end as tax_amount, 
                case when D.paid_amount is null then 0 else D.paid_amount end as paid_amount , D.payment_mode,D.cheque_no,D.account_number ,D.tds_amount,(Select pr_client_id from contact_master cm join purchase_ownership_details p 
                where p.purchase_id=D.property_id and cm.c_id=p.pr_client_id Limit 0,1) as pr_client_id from 
            (select * from 
            (select A.* from 
            (select *
            from actual_other_schedule A where A.gp_id = '$gid' and 
            ...
            on (E.payer_id=F.c_id)) E 
            left join bank_master b on b.b_id=E.account_number
                    left join expense_category_master em on em.id = E.event_name where type='payment'".$where_condition;
    $queryresult = $this->db->query($query);  
    ...
    if(count($result)>0)
    {
        ...
        for($i=0;$i<count($result);$i++)
        {
            ...
            $cheque_no = $result[$i]['cheque_no'];
            $b_name = $result[$i]['b_name'];
            $event_type = $result[$i]['expense_category'];//$result[$i]['expense_category'];
            $paid_amount = $result[$i]['basic_cost'];
            $tax_amount = $result[$i]['tax_amount'];
            $tds_amount = $result[$i]['tds_amount'];
            /*if( $ownerid = "")
            {
            ...
            /*$expense_category_master  = $this->db->query("Select expense_category from expense_category_master where g_id=$gid and id=$category")->result_array();
             $categoryname = $expense_category_master[0]['expense_category'];*/
                $objPHPExcel->getActiveSheet()->setCellValue('B'.$row, $p_property_name);
                 $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $owner_name);
                 $objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $event_type);
        ...
        $sql1="select E.* from 
        (select E.*, E.p_builder_name as contact_id, F.c_full_name, F.owner_name ,'Purchase' as type 
                 from 
        (select C.*, '' as sp_name, C.net_amount-C.paid_amount as bal_amount, 
             D.c_full_name as payer_full_name, D.owner_name as payer_owner_name from 
        (select * from 
        (select A.*, B.purchase_id, B.event_type, B.event_name, B.event_date, B.accounting_id, B.txn_status, B.entry_type, B.sub_property_id  ,
            ...
            case when B.paid_amount is null then 0 else B.paid_amount end as paid_amount ,B.payment_mode ,B.cheque_no,B.account_number,B.tds_amount ,(Select pr_client_id from contact_master cm join purchase_ownership_details p 
                where p.purchase_id=B.purchase_id and cm.c_id=p.pr_client_id Limit 0,1) as pr_client_id from 
        ...
        (select A.purchase_id, A.event_type, A.event_name, A.event_date, A.basic_cost, A.net_amount, A.tax_amount, 
            B.id as accounting_id, B.txn_status, case when B.paid_amount is null then 0 else B.paid_amount end as paid_amount, A.entry_type ,B.payment_mode ,B.cheque_no,B.account_number,B.tds_amount ,B.sub_property_id  from 
        ...
        left join 
        (select id, fk_txn_id, event_type, event_name, event_date, txn_status, paid_amount+tds_amount as paid_amount ,payment_mode,cheque_no,account_number,tds_amount ,sub_property_id 
        ...
        select id, fk_txn_id, tax_applied as event_type, 'tax' as event_name, created_on as event_date, txn_status, 
            amount_paid as paid_amount ,payment_mode,cheque_no,account_number,'' as tds_amount , '' as   sub_property_id from actual_schedule_taxes where table_type = 'purchase') B 
        ...
        from contact_master A left join contact_master B on (A.c_contact_id=B.c_id) 
        where A.c_status='Approved' and A.c_gid='$gid') F 
        on (E.p_builder_name=F.c_id)) E Where type='Purchase' ".$where_condition;
        $result = [];
        ...
        for($i=0;$i<count($result);$i++)
        {
            ...
            $p_property_name = $result[$i]['p_property_name'];
            $owner_name= $result[$i]['owner_name'];
            $payer_name= $result[$i]['payer_owner_name'];
            $payment_mode = $result[$i]['payment_mode'];
            $cheque_no = $result[$i]['cheque_no'];
            $paid_amount = $result[$i]['basic_cost'];
            $tax_amount = $result[$i]['tax_amount'];
       ...
       if(count($queryresult)>0 || count($result1)>0)
        {
            $count = count($result); 
...
public function expense_copy(){
    ...
    if($from_date!="" && $to_date!="")
        ...
        $query = $this->db->query("Select p.p_property_name,p.txn_id ,a.*,sp_name,
            case when c_owner_type='individual' 
            then concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) 
            else concat(ifnull(c_company_name,'')) end as c_name,b.b_name,b.b_accountnumber
            from  actual_other_schedule a  
            join  purchase_txn p  on a.property_id=p.txn_id
            join sub_property_allocation s on s.property_id=p.txn_id
            join purchase_ownership_details po on p.txn_id=po.purchase_id
                ...
                for($j=0;$j<count($resultowner);$j++)
                 {
                      ...
                      $sql =  "Select leasedeatil.*,p_property_name,p_address,p_propertydescription,p_city,case when c_owner_type='individual' 
                        then concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) 
                        else concat(ifnull(c_company_name,'')) end as c_name  from (
                        select E.*,sum(E.net_amount-E.paid_amount) as balamount,sum(E.net_amount) as net,sum(E.paid_amount) as paid,E.eventname,
                                  ...
                                  if($prev!=trim($result[$i]['property_id']))
                                    {
                                        ...
                                        $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,"Paid Amount");
...
function get_owners(){
    ...
    from contact_master A left join contact_master B on (A.c_contact_id=B.c_id) 
    where A.c_status='Approved' and A.c_gid='$gid') B 
    on (A.pr_client_id=B.c_id) $propertyid";
    $query=$this->db->query($sql);
    $result=$query->result();
    return $result;
}
